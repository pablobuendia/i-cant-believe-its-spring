 ...
 *
 */
@ExtendWith(SpringExtension.class) // Integrates the Spring 5 Test Context framework with
									// JUnit 5
@Tag("Controller")
public class UserAccountControllerTest {
	...
	public void setUp(TestInfo testInfo) throws Exception {
		this.mockMvc = MockMvcBuilders
				.standaloneSetup(new UserAccountController(this.userService)).build();
	}
	...
	public void should_ReturnErrorMessage_ForUnmatchingUser() throws Exception {
		...
		//
		Mockito.when(this.userService.isValidUser("foo@bar.com", "foobar"))
				.thenReturn(null);
		//
		// Invoking the controller method
		//
		MvcResult result = this.mockMvc
				.perform(post("/account/login/process")
		...
		ExecutionStatus responseObj = mapper.readValue(response.getContentAsString(),
				ExecutionStatus.class);
		assertTrue(responseObj.getCode().equals("USER_LOGIN_UNSUCCESSFUL"));
		assertTrue(responseObj.getMessage()
				.equals("Username or password is incorrect. Please try again!"));
	}
}
